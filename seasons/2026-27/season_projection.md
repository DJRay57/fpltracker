# Season Projection (through GW5, 5,000 simulations)

_Monte Carlo simulation. Gameweeks already played are exact. For the rest, each trial first draws how good every manager actually is -- after 5 gameweeks the table is 38% signal, so that is mostly guesswork and the simulation treats it that way. A squad's current edge decays as squads churn and stops counting at the GW24 re-draft, when everyone starts again._
